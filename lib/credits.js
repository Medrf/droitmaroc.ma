...
const UNLIMITED_EMAILS = ['amine.rifai.123@gmail.com']

async function isUserUnlimited() {
    try {
        const user = await currentUser()
        if (!user) return false
        return user.emailAddresses?.some(e => UNLIMITED_EMAILS.includes(e.emailAddress))
    } catch (e) {
        return false
    }
}
...
export async function deductCredits(userId, cost, feature) {
    ...
    if (!supabaseAdmin) return { success: false, error: 'System configuration error' }

    if (await isUserUnlimited()) {
        await logUsage(userId, feature, cost, 'success_unlimited')
